...
main = do
  c <- connectPostgreSQL "host=localhost dbname=Source1 user=patrick password=inesh69"
  ...
  print allAuthors
  newAuthorId <- createAuthor "Bohdan" "Khomych" newSourceId c
  print newAuthorId
  newAuthor <- readAuthor c newAuthorId
  print newAuthor
  updateAuthor newAuthorId "Bohdan" "Khomych" newSourceId c
